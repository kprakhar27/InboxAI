...
class TestEmailPreprocessTasks(unittest.TestCase):
    ...
    @patch('data_pipeline.airflow.dags.tasks.email_preprocess_tasks.os.makedirs', return_value=None)
    ...
    def test_preprocess_emails(self, mock_makedirs, mock_email_preprocessor_class):
        ...
        with patch('pandas.Series.apply', return_value=pd.Series(['decoded1', 'decoded2'])):
            # Mock context
            mock_context = {
                'ti': MagicMock(),
                'ds': '2023-01-01',
                'dag_run': MagicMock(conf={'gcs_uri': 'gs://test-bucket/raw/test-user/test'})
            }
        ...
        for call in calls:
            ...
            if 'key' in kwargs and kwargs['key'] == 'preprocessing_metadata':
                if 'value' in kwargs and kwargs['value'].get('execution_date') == '2023-01-01':
                    expected_call_found = True
                    break
        
        self.assertTrue(expected_call_found, "Expected xcom_push call with preprocessing_metadata not found")
    ...
    def test_upload_processed_to_gcs(self, mock_read_parquet, mock_storage_client):
        ...
        mock_context = {
            ...
            'ds': '2023-01-01',
            'dag_run': MagicMock(
                conf={'gcs_uri': 'gs://test-bucket/raw/user123/test-run-id/test@example.com'}
            )
        }
        ...
        ti.xcom_push.assert_called_with(
            key="processed_gcs_uri", 
            value=result
        )
        
    @patch('data_pipeline.airflow.dags.tasks.email_preprocess_tasks.TriggerDagRunOperator')
    def test_trigger_embedding_pipeline(self, mock_trigger_operator):
        ...
        # Mock context
        mock_ti = MagicMock()
        mock_ti.xcom_pull.return_value = 'gs://test-bucket/processed/user123/test-run-id/test@example.com'
        ...
        self.assertEqual(trigger_call_args['trigger_dag_id'], 'email_embedding_generation_pipeline')
