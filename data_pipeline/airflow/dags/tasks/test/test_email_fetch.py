...
class TestEmailFetchTasks(unittest.TestCase):
    ...
    @patch(
        "data_pipeline.airflow.dags.tasks.email_fetch_tasks.update_last_read_timestamp"
    )
    ...
    def test_process_emails_batch(
        ...
        mock_update_last_read_timestamp,
        mock_save_emails,
        mock_validate_emails,
        mock_retrieve_email_data,
        mock_gmail_service,
        ...
        mock_context = {"task_instance": mock_ti, "ti": mock_ti}
        ...
        mock_authenticate_gmail.assert_called_once_with(
            mock_session, "test@example.com"
        )
        mock_retrieve_email_data.assert_called_once_with(
            mock_gmail, ["msg1", "msg2", "msg3"]
        )
    ...
    def test_upload_raw_data_to_gcs(
        self, mock_getenv, mock_exists, mock_storage_service
    ):
        ...
        mock_context = {
            "ti": mock_ti,
            "dag_run": MagicMock(
                conf={"email_address": "test@example.com", "user_id": "user123"}
            ),
        }
        ...
        mock_storage.get_emails_dir.return_value = (
            "/tmp/emails/test@example.com/test-run-id"
        )
        ...
        # Verify results
        self.assertEqual(
            result, "gs://test-bucket/raw/user123/test-run-id/test@example.com"
        )
        mock_storage.get_emails_dir.assert_called_once_with(
            "test@example.com", "test-run-id", "user123"
        )
        mock_storage.upload_directory_to_gcs.assert_called_once()
        mock_ti.xcom_push.assert_any_call(
            key="gcs_uri",
            value="gs://test-bucket/raw/user123/test-run-id/test@example.com",
        )
        mock_ti.xcom_push.assert_any_call(key="upload_stats", value=mock_upload_stats)

    @patch("data_pipeline.airflow.dags.tasks.email_fetch_tasks.get_db_session")
    @patch(
        "data_pipeline.airflow.dags.tasks.email_fetch_tasks.add_preprocessing_summary"
    )
    def test_publish_metrics_task(
        self, mock_add_preprocessing_summary, mock_get_db_session
    ):
        ...
        mock_context = {
            "ti": mock_ti,
            "dag_run": MagicMock(conf={"email_address": "test@example.com"}),
        }
    ...
    def test_validation_task(self):
        ...
        mock_ti.xcom_pull.side_effect = lambda key: (
            mock_processing_metrics
            if key == "email_processing_metrics"
    ...
    def test_trigger_preprocessing_pipeline(self, mock_trigger_operator):
        ...
        mock_ti = MagicMock()
        mock_ti.xcom_pull.return_value = (
            "gs://test-bucket/raw/user123/test-run-id/test@example.com"
        )
